...
@st.cache_data(ttl=5) # 즉각적인 반응을 위해 캐시 시간 단축
def get_records():
...
with st.expander("❌ 예약 취소하기"):
    ...
    if st.button("예약 확인"):
        ...
        if cell:
            ...
            st.session_state['found_row'] = cell.row
            st.info(f"찾은 예약: **{row_data[3]}** 님 (연락처: {row_data[4]})")
        else:
            st.error("해당 번호로 된 예약 기록을 찾을 수 없습니다.")

    if 'found_row' in st.session_state:
        if st.button("진짜 취소하기"):
            sheet.delete_rows(st.session_state['found_row'])
            st.cache_data.clear()
            del st.session_state['found_row']
